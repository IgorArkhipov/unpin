...
- macOS candidates must retain the exact app, bridge, and CLI identifiers and
  ...
  or identifier rotation is rejected as an update-trust violation.
...
upgrade. Both releases use the same certificate and identifiers, so the
replacement preserves the designated-requirement trust boundary. Rebuilt
executables could still prompt for Keychain access. Compatible releases after
`v1.1.0` can use the built-in CLI or desktop update flow.
