...
The exact preceding implementation commit passed the full locked Rust
workspace gates, the shared 35-test Xcode scheme, all release-tool tests, native
arm64 and x86_64 desktop archive smokes, live Pi/OpenCode host validation, and a
finalized repository-local provider matrix with 31/31 CLI, 31/31 TUI, and 31/31
MCP cases.

The `1.0.0` promotion changes only version metadata, release policy, and
documentation. Before publication, the exact promotion commit must pass locked
metadata and workspace gates, release-tool tests, workflow lint, Linux release
compatibility, and exact-version desktop artifact verification.
