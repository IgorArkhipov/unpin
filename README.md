...
Unpin `0.1.0-beta.2` is the first public beta. GitHub Releases provide
...
```bash
gh attestation verify unpin-v0.1.0-beta.2-TARGET.tar.gz \
...
MCP tool IDs retain the predecessor-compatible `agentscope_` prefix. Tool titles, descriptions, and server identity use Unpin branding.
...
```bash
request='{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"agentscope_plan_toggle_items","arguments":{"selector":{"providers":["claude"],"kinds":["plugin"]},"targetEnabled":false}}}'
printf '%s\n' "$request" \
  | cargo run -p unpin-cli -- mcp --fixture-root crates/unpin-core/tests/fixtures --once
```

MCP apply tools require exact reviewed fingerprints and return structured human-action handoffs without writing provider state. Bulk requests also require `maxItems`. Configured MCP entries named `unpin` or `agentscope` remain protected from disable attempts through their own control plane.
