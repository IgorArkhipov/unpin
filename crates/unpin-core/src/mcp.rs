...
impl McpDiscoveryCache {
    ...
    fn get_or_discover(&self, roots: &DiscoveryRoots) -> Result<Arc<DiscoveryOutput>, String> {
        self.get_or_update(|| discover_all(roots).map_err(|error| error.to_string()))
    }

    fn refresh(&self, roots: &DiscoveryRoots) -> Result<Arc<DiscoveryOutput>, String> {
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct McpAgentPluginComponentView {
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct McpAgentPluginActivationCoverage {
    total: usize,
    enabled: usize,
    disabled: usize,
    writable: usize,
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct McpAgentPluginInstanceView {
    provider: ProviderId,
...
fn list_agent_plugins(context: &McpContext, arguments: &Value) -> Result<Value, String> {
    require_empty_object(arguments)?;
...
fn inspect_agent_plugin(context: &McpContext, arguments: &Value) -> Result<Value, String> {
    require_only_fields(
        arguments,
        &["logicalId"],
...
fn plan_agent_plugin_toggle(context: &McpContext, arguments: &Value) -> Value {
...
fn mcp_agent_plugin_value(package: &AgentPluginSummary, include_instances: bool) -> Value {
...
fn mcp_agent_plugin_components(
...
fn mcp_agent_plugin_instance(instance: &AgentPluginInstance) -> McpAgentPluginInstanceView {
...
fn agent_plugin_plan_review_value(package: &AgentPluginSummary, plan: &BulkTogglePlan) -> Value {
    let included = plan
        .included
        .iter()
        .map(|item| {
...
fn agent_plugin_plan_coverage_value(package: &AgentPluginSummary, plan: &BulkTogglePlan) -> Value {
    Value::Array(
        package
            .instances
            .iter()
...
fn agent_plugin_plan_error_value(
    package: &AgentPluginSummary,
...
fn agent_plugin_not_found_value() -> Value {
    json!({
        "status": "blocked",
...
fn agent_plugin_discovery_error_value() -> Value {
    json!({
        "status": "blocked",
...
fn invalid_arguments_value(reason: String) -> Value {
    json!({
        "status": "blocked",
...
const fn agent_plugin_guidance(access: AgentPluginAccess) -> &'static str {
    match access {
...
const fn agent_plugin_lifecycle_guidance(lifecycle: ProviderReachLifecycle) -> &'static str {
    match lifecycle {
...
const fn provider_reach_lifecycle_name(lifecycle: ProviderReachLifecycle) -> &'static str {
    match lifecycle {
...
fn agent_plugin_provider_reach_schema(
    provider_ids: &[&str],
