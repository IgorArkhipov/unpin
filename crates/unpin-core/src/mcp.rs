...
// Keep predecessor-era IDs stable for existing MCP client configurations.
...
fn handle_tool_call(context: &McpContext, request: &Value) -> Result<Value, String> {
    ...
    match name {
        ...
        "agentscope_list_items" => structured_result(list_items(context, &arguments)?),
        "agentscope_run_doctor" => structured_result(run_doctor_structured(context)),
        "agentscope_plan_toggle_item" => structured_result(plan_single_toggle(context, &arguments)),
        "agentscope_apply_toggle_item" => {
            structured_result(apply_single_toggle(context, &arguments))
        }
        "agentscope_plan_toggle_items" => {
            structured_result(plan_bulk_toggle_items(context, &arguments))
        }
        "agentscope_apply_toggle_items" => {
            structured_result(apply_bulk_toggle_items(context, &arguments))
        }
        "agentscope_list_backups" => structured_result(list_backups(context, &arguments)),
        "agentscope_restore_backup" => structured_result(restore_backup_tool(context, &arguments)),
...
fn plan_single_toggle(context: &McpContext, arguments: &Value) -> Value {
    ...
    plan["operation"] =
        serde_json::to_value(operation).expect("control operation envelope serializes");
    plan["continuation"] =
        json!("Review this plan, then call agentscope_apply_toggle_item with its planFingerprint.");
    plan
}
...
fn is_control_plane_name(item: &DiscoveryItem) -> bool {
    ...
    matches!(id_name.as_str(), "unpin" | "agentscope")
        || matches!(display_name.as_str(), "unpin" | "agentscope")
}
...
fn restore_backup_tool(context: &McpContext, arguments: &Value) -> Value {
    ...
    let mut response = if reviewed {
        ...
        json!({
            ...
            "planFingerprint": fingerprint,
            "operation": operation,
            "continuation": "Review this plan, then call agentscope_restore_backup again with its planFingerprint."
        })
    };
...
fn tool_title(name: &str) -> &'static str {
    match name {
        "agentscope_get_inventory_summary" => "Get Unpin inventory summary",
        "agentscope_list_items" => "List Unpin items",
        "agentscope_plan_toggle_item" => "Plan one Unpin item toggle",
        "agentscope_apply_toggle_item" => "Request one Unpin item toggle",
        "agentscope_plan_toggle_items" => "Plan Unpin item toggles",
        "agentscope_apply_toggle_items" => "Request Unpin item toggles",
        "agentscope_list_backups" => "List Unpin backups",
        "agentscope_restore_backup" => "Request Unpin backup restore",
        "agentscope_run_doctor" => "Run Unpin doctor",
...
fn tool_description(name: &str) -> &'static str {
    match name {
        ...
        "agentscope_list_backups" => "List recent Unpin mutation backups from local app state.",
        "agentscope_restore_backup" => {
            "Validate one backup restore request and return a human-action handoff without writing."
        }
        "agentscope_run_doctor" => {
            "Return structured fixture and provider discovery health output."
        }
        "unpin_get_control_status" => {
...
fn tool_input_schema(name: &str) -> Value {
    ...
    match name {
        "agentscope_get_inventory_summary" => json!({
            "type": "object",
            "properties": {
        ...
        "agentscope_plan_toggle_item" => json!({
            "type": "object",
        ...
        "agentscope_apply_toggle_item" => json!({
            "type": "object",
        ...
        "agentscope_plan_toggle_items" => json!({
            "type": "object",
        ...
        "agentscope_apply_toggle_items" => json!({
            "type": "object",
        ...
        "agentscope_restore_backup" => json!({
            "type": "object",
            "required": ["backupId"],
            "properties": {
