...
fn mcp_without_backup_key_allows_plans_and_hands_off_apply_without_writing() {
    ...
    let summary = call_tool(&context, "agentscope_get_inventory_summary", json!({}));
    ...
    let planned = call_tool(&context, "agentscope_plan_toggle_item", arguments.clone());
    assert_eq!(planned["status"], "planned");
...
fn bulk_apply_requires_fingerprint_and_max_items_but_not_boolean_confirmation() {
    ...
    let planned = call_tool(&context(), "agentscope_plan_toggle_items", selector.clone());
...
fn bulk_apply_with_matching_fingerprint_returns_handoff_without_writes() {
    ...
    let planned = call_tool(&context, "agentscope_plan_toggle_items", request.clone());
...
#[test]
fn descriptors_preserve_nine_legacy_ids_and_brand_new_control_tools_as_unpin() {
    let response = handle_mcp_request(
        &context(),
        &json!({
            "jsonrpc": "2.0",
    ...
    for (index, tool) in response["result"]["tools"]
        .as_array()
        .expect("tools array")
        .iter()
        .enumerate()
    {
        ...
        if index < 9 {
            assert!(name.starts_with("agentscope_"), "legacy tool id changed");
        } else {
            assert!(name.starts_with("unpin_"), "new tool id lacks brand");
        }
        assert!(
            tool["title"]
                .as_str()
        ...
        assert!(
            !tool["description"]
                ...
                .contains("AgentScope"),
            "tool description should not expose predecessor branding"
        );
    }
}
...
fn bulk_apply_descriptor_requires_review_fields() {
    ...
    let plan = tool_descriptor(&context, "agentscope_plan_toggle_items");
    assert_eq!(required_input_fields(&plan), vec!["targetEnabled"]);

    let apply = tool_descriptor(&context, "agentscope_apply_toggle_items");
    assert_eq!(
...
fn descriptors_constrain_known_input_values() {
    ...
    let summary = tool_descriptor(&context, "agentscope_get_inventory_summary");
    ...
    let single = tool_descriptor(&context, "agentscope_plan_toggle_item");
    ...
    let list = tool_descriptor(&context, "agentscope_list_items");
    ...
    let backups = tool_descriptor(&context, "agentscope_list_backups");
    assert_eq!(
    ...
    let restore = tool_descriptor(&context, "agentscope_restore_backup");
    assert_eq!(
...
fn control_plane_configured_mcp_disable_is_blocked_over_single_mcp_tools() {
    ...
    let planned = call_tool(&context, "agentscope_plan_toggle_item", selector.clone());
...
fn returns_inventory_summary_and_doctor_structured_content() {
    let summary = handle_mcp_request(
        ...
        &json!({
            ...
            "method": "tools/call",
            "params": {
                "name": "agentscope_get_inventory_summary",
                "arguments": {}
            }
        }),
    );
...
fn mcp_backup_summaries_mark_invalid_restore_manifests_unrestorable() {
    ...
    let backups = call_tool(&context, "agentscope_list_backups", json!({}));
    ...
    let limited = call_tool(&context, "agentscope_list_backups", json!({ "limit": 2 }));
...
fn plans_claude_connector_plugin_and_hands_off_without_writing() {
    ...
    let unconfirmed = call_tool(&context, "agentscope_apply_toggle_item", selector.clone());
    assert_eq!(unconfirmed["status"], "blocked");
    assert_eq!(
        unconfirmed["reason"],
        "plan fingerprint does not match current reviewed plan"
