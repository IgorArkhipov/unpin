...
#[test]
fn recursive_project_skill_scans_exclude_repository_tmp() {
    let home_root = tempfile::TempDir::new().expect("temp home root");
    let repository_root = tempfile::TempDir::new().expect("temp repository root");
    let cursor_root = tempfile::TempDir::new().expect("temp cursor root");
    write_file(
    ...
    for (relative_path, heading) in [
        (
            "scratch/.claude/skills/ordinary-claude/SKILL.md",
            "Ordinary Claude",
        ),
        (
            "scratch/.cursor/skills/ordinary-cursor/SKILL.md",
            "Ordinary Cursor",
        ),
        (
            "tmp/private/.claude/skills/private-claude/SKILL.md",
            "Private Claude",
        ),
        (
            "tmp/private/.cursor/skills/private-cursor/SKILL.md",
            "Private Cursor",
        ),
    ] {
        write_file(
    ...
    for id in [
        "claude:project:skill:@scope/scratch/ordinary-claude",
        "cursor:project:skill:@scope/scratch/ordinary-cursor",
    ] {
        assert!(
            result.items.iter().any(|item| item.id == id),
            "ordinary project skill {id} should remain discoverable; got {:#?}",
            result.items
        );
    }
    assert!(
        result.items.iter().all(|item| {
            Path::new(&item.source_path)
                ...
                .map_or(true, |relative| !relative.starts_with("tmp"))
        }),
        "repository tmp content must not become live project capabilities; got {:#?}",
        result.items
    );
}
