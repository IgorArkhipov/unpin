...
#[test]
#[cfg(unix)]
fn live_profile_apply_without_session_authority_fails_before_mutation() {
    ...
    assert!(
        output.contains("session authority key missing; run `unpin auth session init`"),
...
fn mcp_restore_plan_fingerprint_is_accepted_by_cli_apply() {
    ...
    let request = line_request(serde_json::json!({
        ...
        "method": "tools/call",
        "params": {
            "name": "agentscope_restore_backup",
...
fn mcp_once_plans_single_skill_toggle() {
    ...
    let request = line_request(serde_json::json!({
        ...
        "method": "tools/call",
        "params": {
            "name": "agentscope_plan_toggle_item",
            "arguments": {
                "provider": "claude",
                "kind": "skill",
                "layer": "project",
